...
class HealingScriptTransformer:
    ...
    def transform(
        ...
        Args:
            ...
            assert_stubs: 断言桩列表
            target_url: 录制时的目标业务 URL（用于替换登录页 goto）
...
Module: {module_name}
Self-healing enabled via healing_page fixture

Run with:
    pytest {Path(output_path).name} -v
"""
...
'''
        ...
        # 7d. 替换业务路由
        source = self._replace_with_business_route(source, target_url)
...
自愈功能由 conftest.py 的 healing_page fixture 提供。
"""
...
'''
    ...
    def _replace_with_business_route(self, source: str, target_url: str = "") -> str:
        """如果首个 goto 是登录页，删除登录流程，替换为 goto 业务页"""
        lines = source.split('\n')

        # 1. 找到第一个 goto，检查是否是登录页
        first_goto_idx = -1
        for i, line in enumerate(lines):
            if line.strip().startswith('healing_page.goto('):
                first_goto_idx = i
                if not ('login' in line.lower() or 'user-login' in line.lower() or 'WEB_DEMAND_LOGIN_PAGE_URL' in line):
                    return source  # 不是登录页，不处理
                break

        if first_goto_idx < 0:
            return source

        # 2. 找到登录按钮点击行（包含"登录"或"登 录"的 click）
        login_click_idx = first_goto_idx
        for i in range(first_goto_idx + 1, len(lines)):
            if ('登' in lines[i] and '录' in lines[i]) and '.click()' in lines[i]:
                login_click_idx = i
                break

        # 3. 删除从 goto 到登录按钮点击的所有行，替换为业务页 goto
        if target_url:
            business_goto = f'    healing_page.goto(os.environ.get("WEB_DEMAND_URL", "{target_url}"), timeout=30000, wait_until="domcontentloaded")'
        else:
            business_goto = '    healing_page.goto(os.environ.get("WEB_DEMAND_URL"), timeout=30000, wait_until="domcontentloaded")'

        new_lines = lines[:first_goto_idx] + [business_goto] + lines[login_click_idx + 1:]
        print(f"   🔗 删除 {login_click_idx - first_goto_idx} 行登录流程，替换为业务页导航")
        return '\n'.join(new_lines)
